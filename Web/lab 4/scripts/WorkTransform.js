export default class WorkTransform{
    ...
    _beginAnim() {
        ...
        this.square2.style.display = "block"
    ...
    _action = () => {
        ...
        this.square2.style.transform = `translate(${this.x[1]}px,${this.y[1] - 15}px)`
        ...
        if(isUpper || isLower) {
            ...
            this.square2.style.display = "none"
        ...
        for(let i = 0; i <this.x.length; i++) {
            if(this.x[i] + 1 * this.deg[i] + 15 > this.canvas.width && this.direction[i][0] == 1) {
                this.y[i] += (this.x[i] - this.canvas.width) / this.deg[1] * this.direction[i][1]
                this.x[i] = this.canvas.width - 15
            ...
            } else if(this.y[i] + 15 > this.canvas.height) {
                this.direction[i][1] = -1
                this.y[i] = this.canvas.height - 15
